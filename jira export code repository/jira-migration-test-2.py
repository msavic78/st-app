...
def update_links(row, key_map):
    ...
    epic_link_column = [col for col in row.index if 'Custom field (Epic Link)' in col]
...
# Identify all possible inward and outward columns, including Epic Link
...
all_epic_link_columns = [col for col in combined.columns if 'Custom field (Epic Link)' in col]

# Ensure all inward, outward, and epic link columns are present in the final file1 dataframe
...
for col in all_epic_link_columns:
    if col not in file1.columns:
        file1[col] = None
...
unchanged_epic = file1[file1.filter(regex='Custom field \(Epic Link\)').apply(lambda x: x.str.contains('AHA', na=False)).any(axis=1)]
...
logging.info("Rows with unchanged Custom field (Epic Link):")
logging.info(unchanged_epic[['Summary'] + list(unchanged_epic.filter(regex='Custom field \(Epic Link\)').columns)].to_string(index=False))

# After updating the inward/outward links and epic link, update the Issue Key column
...
output_file = 'Updated-Blocked-Epics.xlsx'
